...
function getStudents($Programme,$year,$intake) {
        ...
        if (!$query_run) {
            ...
            if ($num>0){
                    ...
                    while ($row = mysqli_fetch_array($query_run)) {
                    echo "<option value='".$row['id']."'><b>".$row['studentNo']." :: ".ucfirst($row['surname'])."".$row['firstName']."</option></b>";   
                }
            }else{
...
function loadTranscript($id,$sel_year,$sel_semester) {
    $no_content = "<div class='jumbotron'>No Results Found!</div>";
    $con = con();
    $get_details = mysqli_query($con, "SELECT * FROM student, marks WHERE student.id = marks.student_studentID AND student.id='$id' AND marks.semester='$sel_semester' AND marks.yearOfOffering='$sel_year'");
    if(!$get_details){
        ...
        <div class='col-lg-12 text-center'>
            ...
            <div class='panel panel-primary'>
                ...
                <div class='panel-body text-left'>
                    ...
                    <div class='col-md-6 transcript-info '>
                        <div><b>$caption_program:</b> &nbsp; $prog_name </b></div>
                    </div>
                </div>
            </div>
        </div> 
