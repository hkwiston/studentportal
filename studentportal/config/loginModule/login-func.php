...
function login($username, $password, $mysqli) {
    if ($stmt = $mysqli->prepare("SELECT id, regNo, password FROM student WHERE regNo = ? LIMIT 1")) {
        $stmt->bind_param('s', $username);  // Bind "$username" to parameter.
        $stmt->execute();    // Execute the prepared query.
        $stmt->store_result();
        // get variables from result.
        $stmt->bind_result($user_id, $registration_number, $db_password);
        $stmt->fetch();
        if ($stmt->num_rows == 1) {
...
function allow_login(){
    ...
    $dob = getfield("designation",$mysqli);
    $group = getfield("group",$mysqli);
    if (empty($fname) || empty($sname) || empty($designation) || empty($dob)){
        return true;
    }else{
        return false;
    }
